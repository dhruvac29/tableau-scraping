...
class TableauScraper:
    ...
    def loads(self, url, params={}):
        ...
        if tableauPlaceHolder is not None:
            ...
            if (
                ("host_url" not in params)
                or ("site_root" not in params)
            ...
            url = (
                f'{params["host_url"][:-1]}{params["site_root"]}/views/{params["name"]}'
            )
        ...
        if container and container.text:
            ...
            self.tableauData = self.session.post(
                surl, params=params, verify=self.verify
            ).json()
            # ?:showVizHome=n
        ...
        try:
            ...
            if "presModelMap" in self.data["secondaryInfo"]:
                ...
                self.dataSegments = presModelMap["dataDictionary"]["presModelHolder"][
